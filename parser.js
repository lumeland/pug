import { assert, TokenStream } from "./deps.js";
import error from "./error.js";
import inlineTags from "./lib/inline-tags.js";
...
Parser.prototype = {
  ...
  parseInclude: function () {
    ...
    if (
      ...
      node.block = "indent" == this.peek().type
        ? this.block()
        : this.emptyBlock(tok.loc.start.line);
  ...
  tag: function (tag, options) {
    ...
    switch (this.peek().type) {
      ...
      case ":":
        this.advance();
        var expr = this.parseExpr();
        tag.block = expr.type === "Block"
          ? expr
          : this.initBlock(tag.line, [expr]);
        break;
      case "newline":
      case "indent":
      case "outdent":
